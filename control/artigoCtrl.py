...
class ArtigoCtrl:
    ...
    def salvar_atualizar_artigo(self, id=None, titulo=None, palavras_chave=None, autor=None):
        try:
            ...
            if id:
                ...
                artigo = Artigo(nome=titulo, palavras_chave=palavras_chave, autor=A)
            artigo.save()
    ...
    def buscar_artigo(self, id=None, nome=None):
        try:
            if id:
                artigo = Artigo.get_by_id(id)
            elif nome:
                artigo = Artigo.select().where(Artigo.nome % f'%{nome}%')
            else:
        ...
        if type(artigo) is Artigo:
            itens.append(self._montar_artigo(artigo.id, artigo.nome, artigo.palavras_chave, artigo.autor.nome))
        elif type(artigo) is ModelSelect:
            for a in artigo:
                itens.append(self._montar_artigo(a.id, a.nome, a.palavras_chave, a.autor.nome))
        return itens

    def _montar_artigo(self, id, nome, palavras_chave, autor):
        meuartigo = {
